...
import { postTilawah } from '../../../fetch/api/Teacher';
...
const CreateTilawah = () => {
  ...
  const handleClick = async () => {
    if (token) {
      ...
      const { status } = await postTilawah(surah, juz, ayat, decryptedToken);
      if (status === 200) {
        setIsError((prev) => (prev === false ? prev : !prev));
        setMessage('Berhasil membuat tilawah');
      } else {
        setIsError((prev) => (prev === true ? prev : !prev));
        setMessage('Gagal membuat tilawah');
      }
    }
  };
