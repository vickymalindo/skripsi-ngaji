...
import { editMurojaah, getDetailMurojaah } from '../../../fetch/api/Teacher';
...
const EditMurojaah = () => {
  ...
  const handleClick = async () => {
    ...
    let ayat = '' + ayatState;
    const res = await editMurojaah(surah, juz, ayat, id);
    const { status } = res;
    if (status === 200) {
      setIsError((prev) => (prev === false ? prev : !prev));
      setMessage('Berhasil edit hafalan lama');
    } else {
      setIsError((prev) => (prev === true ? prev : !prev));
      setMessage('Gagal edit hafalan lama');
    }
  };
  ...
  React.useEffect(() => {
    (async function getSurah() {
      ...
      const getDataMurojaah = await getDetailMurojaah(id);
